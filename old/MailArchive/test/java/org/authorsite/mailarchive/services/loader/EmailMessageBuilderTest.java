 ...
 */
public class EmailMessageBuilderTest extends AbstractPersistenceTestCase {
    ...
    private static Logger logger = Logger.getLogger(EmailMessageBuilderTest.class);
    
    protected void setUp() throws MessagingException {
        
        addressDAO = new MemoryEmailAddressDAO();
        messageDAO = new MemoryEmailMessageDAO();
        builder = new EmailMessageBuilder();
        builder.setEmailAddressDAO(addressDAO);
        builder.setEmailMessageDAO(messageDAO);
        
        Properties props = new Properties();
        session = Session.getDefaultInstance(props);
        URLName name = new URLName("mbox://" + System.getProperty("java.io.tmpdir") );
        store = session.getStore(name);
        testing = store.getFolder(new URLName(System.getProperty("java.io.tmpdir") + File.separator + "testing1.mbox"));
        
        // testing bit
        testing.open(Folder.READ_ONLY);
    }
    ...
    public EmailMessageBuilderTest (String name) throws Exception {
        super(name);
    }
    
    public static void main(String[] args) {
        junit.textui.TestRunner.run(EmailMessageBuilderTest.class);
    }
